...
class userdisplay implements displayable {
    ...
    public static function create($user, $context = null): userdisplay {
        return new userdisplay($user, $context);
    }
    ...
    // get details based on field name
    public function get_field($field, array $options = []): string {

        // Check if method exists.
        if (method_exists($this, 'get_' . $field)) {
            // Return the method.
            return $this->{'get_' . $field}($options);
        }

        // Return the user field if the there is no custom method to get the field data.
        return $this->user->$field ?? '';
    }

    // Magic function to get the user field data (without option).
